...
import contrib.net.sf.samtools.BAMRecordCodec;
...
import contrib.net.sf.samtools.SAMRecordQueryNameComparator;
...
import contrib.net.sf.samtools.util.DelegatingIterator;
import contrib.net.sf.samtools.util.SortingCollection;
...
*                                          in which none are primary, or more than one is marked primary
    ...
    public SamAlignmentMerger(final File unmappedBamFile, final File targetBamFile, final File referenceFasta,
        ...
        if (getProgramRecord() != null) {
            try (SAMFileReader tmp = new SAMFileReader(unmappedBamFile)) {
    ...
    @Override
        ...
        if (alignedSamFile != null && alignedSamFile.size() > 0) {
            ...
            final List<SAMFileReader> readers = new ArrayList<>(alignedSamFile.size());
    ...
    private static class SuffixTrimingSamRecordIterator implements CloseableIterator<SAMRecord> {
    ...
    private class SeparateEndAlignmentIterator implements CloseableIterator<SAMRecord> {
        ...
        public SeparateEndAlignmentIterator(final List<File> read1Alignments, final List<File> read2Alignments) {
            ...
            final List<SAMFileReader> read1 = new ArrayList<>(read1Alignments.size());
            final List<SAMFileReader> read2 = new ArrayList<>(read2Alignments.size());
