...
public class IntervalTree<E extends Comparable<E>, T extends HasInterval<E>> extends AbstractCollection<T>
{
	...
	public void check(TreeNode<E,T> treeNode) {
		Stack<TreeNode<E,T>> todo1 = new Stack<>();
		todo1.add(treeNode);
		while (!todo1.isEmpty()) {
			TreeNode<E,T> node = todo1.pop();
			if (node == node.parent) {
				throw new IllegalStateException("node is same as parent!!!");
			}
			if (node.isEmpty()) {
			...
			int leftSize = (node.left != null)? node.left.size:0;
			int rightSize = (node.right != null)? node.right.size:0;
			@Nullable E leftMax = (node.left != null)? node.left.maxEnd:null;
			@Nullable E rightMax = (node.right != null)? node.right.maxEnd:null;
			...
			if (node.left != null) todo1.add(node.left);
			if (node.right != null) todo1.add(node.right);
		}
	}
	...
	final Deque<TreeNode<E,T>> todoStack = new ArrayDeque<>(100);
	...
	public TreeNode<E,T> balance(TreeNode<E,T> node) {
		...
		while (!todoStack.isEmpty()) {
			...
			if (median.left != null) todoStack.push(median.left);
			if (median.right != null) todoStack.push(median.right);
		}
	...
	public void rotateUp(TreeNode<E,T> node, TreeNode<E,T> target) {
		...
		while (n != null && n.parent != null && !done) {
			...
			if (n == n.parent.left) {
				...
				throw new AssertionFailedException("Not on parent's left or right branches.");
			}
			if (debug) check(n);
		}
	}
	...
	// Returns ith node
	public @Nullable TreeNode<E,T> getNode(TreeNode<E,T> node, int nodeIndex) {
		int i = nodeIndex;
		TreeNode<E,T> n = node;
		while (n != null) {
	...
	public List<T> getOverlappingList(T target) {
		return getOverlapping(root, target.getInterval());
	}

	public List<T> getOverlapping(TreeNode<E,T> n, E p)
	{
	...
	public List<T> getOverlapping(TreeNode<E,T> n, Interval<E> target)
	{
		List<T> overlapping = new ArrayList<>(100);
		getOverlapping(n, target, overlapping);
		return overlapping;
	}

	public TreeNode<E,T> getRoot() {
		return root;
	}

	// Search for all intervals which contain p, starting with the
	// node "n" and adding matching intervals to the list "result"
	public void getOverlapping(TreeNode<E,T> n, E p, List<T> result) {
	 ...
	 * @param result
	 */
	public void getOverlapping(Interval<E> target, List<T> result) {
		getOverlapping(getRoot(), target, result);
	}

	//private final ThreadLocal<ResizingArrayQueue<TreeNode<E,T>>> toDoLocal
	//	= ThreadLocal.withInitial(() -> new ResizingArrayQueue<>(5_000));

	private final Queue<TreeNode<E,T>> todo = new ResizingArrayQueue<>(10_000);
	 ...
	 * @param result
	 */
	public void getOverlapping(TreeNode<E,T> node, Interval<E> target, List<T> result) {
		if (!todo.isEmpty()) {
			throw new AssertionFailedException();
		}
		todo.add(node);
