package contrib.it.unimi.dsi.io;
...
import contrib.it.unimi.fastutil.io.MeasurableInputStream;
import contrib.it.unimi.fastutil.io.RepositionableStream;
import contrib.net.sf.samtools.seekablestream.SeekableStream;
...
public class ByteBufferInputStream extends SeekableStream implements RepositionableStream {
	...
	public ByteBufferInputStream( final ByteBuffer byteBuffer ) {
		this(null, new ByteBuffer[] { byteBuffer }, byteBuffer.capacity(), 0, new boolean[ 1 ] );
	}
	...
	protected ByteBufferInputStream(Closeable channel, final ByteBuffer[] byteBuffer, final long size, final int curr, final boolean[] readyToUse ) {
		closeable = channel;
	...
	public static ByteBufferInputStream map( final FileChannel fileChannel, final MapMode mapMode ) throws IOException {
		...
		return new ByteBufferInputStream(fileChannel, byteBuffer, size, 0, readyToUse );
	}

	private ByteBuffer byteBuffer( final int n1 ) {
		if ( readyToUse[ n1 ] ) return byteBuffer[ n1 ];
		readyToUse[ n1 ] = true;
		return byteBuffer[ n1 ] = byteBuffer[ n1 ].duplicate();
	}
	...
	@Override
	public long skip( final long n1 ) throws IOException {
		final long toSkip = Math.min( remaining(), n1 );
	...
	public ByteBufferInputStream copy() {
		return new ByteBufferInputStream(closeable, byteBuffer.clone(), size, curr, new boolean[ n ] );
	}
}
