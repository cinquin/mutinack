...
public class THashMap0<K, V> extends TObjectHash0<K> implements TMap<K, V>, Externalizable {
   ...
   */
  public THashMap0(int initialCapacity) {
   ...
   */
  public THashMap0(int initialCapacity, float loadFactor) {
   ...
   */
  public THashMap0(Map<? extends K, ? extends V> map) {
      this(map.size());
      putAll(map);
  }
   ...
   * @param map a <code>Map</code> value
   */
  public THashMap0(THashMap0<? extends K, ? extends V> map) {
      this(map.size());
      putAll(map);
  }
  ...
  protected class ValueView extends MapBackedView<V> {
      ...
      @SuppressWarnings("OperatorPrecedence")
          ...
          for (int i = 0; i < values.length; i++) {
              if ((set[i] != FREE && set[i] != REMOVED) &&
                      value == values[i] ||
                      (null != values[i] && THashMap0.this.equals(values[i], value))) {
  ...
  protected class EntryView extends MapBackedView<Map.Entry<K, V>> {
      ...
      @Override
          ...
          if (index >= 0) {
              ...
              if (val == _values[index] ||
                      (null != val && THashMap0.this.equals(val, _values[index]))) {
                  removeAt(index);    // clear key,state; adjust size
                  return true;
              }
          }
      ...
      @Override
          ...
          return entryValue == val ||
                  (null != val && THashMap0.this.equals(val, entryValue));
      }
...
//      }
  ...
  protected class KeyView extends MapBackedView<K> {
      ...
      @Override
          ...
          return null != THashMap0.this.remove(key);
      }


      @Override
			public boolean containsElement(K key) {
          return THashMap0.this.contains(key);
      }
  }
